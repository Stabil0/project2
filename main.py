...
class Appw(QWidget):
    ...
    def show_selected_order(self):
        ...
        headers = ['Номер: ', 'Дата: ', 'Время: ', 'Номер клиента: ', 'Усоуги в заказе: ', 'Статус: ']
...
#               Оформление зака
    ...
    def new_order(self):
        ...
        if not id:
            ...
            msg.setText("Номер заказа не введён")
            x = msg.exec_()
            return 0
    ...
    def add_service(self):
        ...
        self.sum_Lb.setText('Итог: '+str(self.serv_sum)+' руб.')
        txt = ""
    ...
    def rem_service(self):
        if self.services:
            ...
            cur_service = list(filter_object)[0]
            self.usluga_id = self.usluga_id[:-1*(len(str(cur_service[0]))+2)]  # эта страшная строка убирает последнию услугу
            self.serv_sum -= cur_service[3]
            self.sum_Lb.setText('Итог: '+str(self.serv_sum)+' руб.')
            txt = ""
            for srv in self.services:
